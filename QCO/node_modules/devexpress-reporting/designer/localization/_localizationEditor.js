...
export class LocalizationEditor extends Disposable {
    ...
    _createLocalizationItem(component, localizedProperty, defaultProperties) {
        var displayName = component.name() + '.' + localizedProperty.propertyName;
        var _defaultText = ko.observable(localizedProperty.value());
        if (!this.isDefaultLanguage()) {
            var defaultTextProperty = defaultProperties.filter(x => x.component === component && x.propertyName === localizedProperty.propertyName)[0];
        ...
        return {
            ...
            visible: ko.computed(() => {
                var regex = new RegExp(this.textToSearch(), 'gi');
    ...
    _updateLocalizationItems() {
        ...
        this._options.report().enumerateComponents().filter(x => this._shouldLocalizeReportControl(x)).forEach((control) => {
            control.getLocalizationProperties().forEach(property => {
                var propertyName = property.info.propertyName;
