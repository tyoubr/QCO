...
export class ReportDesignerInitializer extends CommonDesignerGenerator {
    ...
    addErrorPanelViewModel(element, options) {
        this.addElement('errorPanelViewModel', () => {
            ...
            var rightPosition = ko.observable(0);
            var dispose = null;
            var subscribeDesignMode = () => {
                dispose && dispose();
    ...
    _createPropertiesTab() {
        var getVisible = () => { return !!this.buildingModel.model(); };
        var getDisabled = () => {
            var _a, _b, _c;
    ...
    _createExpressionsTab(context) {
        ...
        var getVisible = () => { return this.buildingModel.model() && this.buildingModel.model()._dataBindingMode() !== DataBindingMode.Bindings; };
        var getDisabled = () => {
    ...
    _createFieldListTab(designerCallbacks, startDataSourceWizard) {
        ...
        var useContextMenu = ko.observable(true);
        var actions = ko.observable([]);
        this.addDisposable(treeListOptions.treeListController.subscribeOnSelectedItemChange(newValue => {
            var _a, _b;
        ...
        var fieldListContextMenu = new ContextMenuProvider({
            actions: actions,
            target: '.' + this.buildingModel.containerClass + ' .dxrd-fieldslist-context-menu-container',
            getClickAtionParams: () => treeListOptions.treeListController.selectedItem,
    ...
    _customizeDesignerActions(designerModel, nextCustomizer, exitDesigner, state, callbacks) {
        ...
        return ((actions) => {
            ...
            var getFullscreenActionImageTemplateName = (fullScreen) => fullScreen ? 'dxrd-svg-toolbar-fullscreen-exit' : 'dxrd-svg-toolbar-fullscreen';
            actions.push({
                ...
                imageTemplateName: getFullscreenActionImageTemplateName(koUtils.unwrap(designerModel.fullScreen)),
                setImageTemplateNameChangedEvent: (callback) => {
                    const handler = (newValue) => callback(getFullscreenActionImageTemplateName(newValue));
                    const dispose = designerModel.fullScreen.subscribe(handler);
                    return () => dispose();
                },
                selected: koUtils.unwrap(designerModel.fullScreen),
                setSelectedChangedEvent: (callback) => {
                    const dispose = designerModel.fullScreen.subscribe(callback);
                    return () => dispose();
                },
                visible: true,
                isContextMenuAction: false,
            ...
            if (reportPreview) {
                let visibilityChangedCallback = null;
                this._addDisposable({
                    dispose: reportPreview.events.on('previewVisibleChanged', (args) => {
                        visibilityChangedCallback && visibilityChangedCallback(args.newValue);
                    })
                });
                var previewAction = {
                    id: ActionId.Preview,
                    ...
                    disabled: ko.pureComputed(() => { return !report(); }),
                    visible: !reportPreview.previewVisible,
                    setVisibleChangedEvent: (callback) => {
                        visibilityChangedCallback = (newValue) => callback(!newValue);
                        return () => {
                            visibilityChangedCallback = undefined;
                        };
                    },
    ...
    _createChartDesignerPart(context) {
        ...
        var chartDesignerOptions = createChartDesignerOptions(this.buildingModel, this.buildingModel.dataSourceHelper, this.buildingModel.model, this.buildingModel.parameters, chartValueBindingProvider, this._accessibilityProvider);
    ...
    addPopularProperties(controlsFactory, accessibilityProvider) {
        ...
        var groupActionsVisible = ko.observable(PropertyGrid.QuickActionsVisible());
