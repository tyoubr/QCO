...
export class ExpressionWrapper extends Disposable {
    ...
    _updateCachedControlInfo(description, useEvents) {
        var _a;
        var displayPath = ((_a = description.displayPath) === null || _a === void 0 ? void 0 : _a.split('.')) || [];
        if (!useEvents) {
    ...
    _createInfo(rootInfo, displayPath, prefix, path) {
        ...
        for (var i = 0; i < path.length; i++) {
            if (typeof (path[i]) !== 'string' && Array.isArray(path[i])) {
                ...
                var pathPart = path[i];
                var propertySplit = pathPart.split('.');
                var propertyName = propertySplit.pop();
                if (propertySplit.length > 0) {
                    ...
                    var currentInfo = info.filter(x => x.propertyName === path[i] || x.propertyName === [prefix, path[i]].join('.'))[0];
                    var displaName = displayPath[i];
                    if (!currentInfo) {
                        var isComplexProperty = i !== path.length - 1;
    ...
    _addControlInfoToCache(controlType, displayPath, ...params) {
        ...
        var cache = this._expressionsSerializationInfoCache[controlType];
        var infos = this._createInfo(cache, displayPath, '', params.filter(x => !!x));
    ...
    _summaryFunctions() {
        ...
        ['Avg', 'Count', 'Sum', 'RunningSum', 'CarryoverSum', 'Percentage', 'Max', 'Min', 'Median', 'Var', 'VarP', 'StdDev', 'StdDevP', 'DAvg', 'DCount', 'DSum', 'DVar', 'DVarP', 'DStdDev', 'DStdDevP', 'RecordNumber',
