...
export class ParameterHelper extends BaseRenderingMultiplatformModel {
    ...
    static getSerializationValue(value, dateConverter) {
        if (value instanceof Array) {
            return value.map((item) => {
                var itemValue = (item === null || item === void 0 ? void 0 : item.value) ? currentMultiPlatformEngine.unwrap(item.value) : item;
    ...
    createInfo(parameter) {
        ...
        var typeString = this.isEnumType(parameter) ? 'Enum' : koUtils.unwrap(parameterDescriptor.type);
    ...
    createMultiValueArray(fromArray, parameter, convertSingleValue) {
        var converter = convertSingleValue ? convertSingleValue : this.getValueConverter(koUtils.unwrap(parameter.type));
    ...
    getEnumCollection(parameter) {
        var type = koUtils.unwrap(parameter.type);
