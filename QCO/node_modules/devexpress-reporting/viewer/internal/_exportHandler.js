...
export class ExportHandler extends BaseRenderingModel {
    ...
    _callPrint(_window) {
        if (_window && ((browser.chrome && !isAndroid) || isIOS)) {
    ...
    _executeXhr(_exportWindow, _requestData, _method, _handleXhrReady) {
        ...
        for (let key in _requestData.FormData) {
    ...
    updateExportStatus(progress, operationId) {
        ...
        if (this._window) {
            ...
            let exportText = getLocalization('Exporting the document...', 'PreviewStringId.Msg_ExportingDocument') + ' ' + progress + '%';
