...
export class MobileReportPreview extends ReportPreview {
    constructor(handlerUri, previewRequestWrapper, previewHandlersHelper, callbacks, rtl = false, mobileSettings = { readerMode: true, animationEnabled: true }, breadcrumb) {
        super(handlerUri, previewRequestWrapper, previewHandlersHelper, callbacks, rtl, undefined, undefined, undefined, breadcrumb);
    ...
    createBrickClickProcessor(cyclePageIndex) {
        ...
        var func = (brick) => {
            ...
            if (cyclePageIndex !== this.pageIndex) {
                this.actionsVisible = false;
                var supscriptionDispose = this.events.on('actionsVisibleChanged', (args) => {
                    supscriptionDispose();
                    this.actionsVisible = false;
                });
            }
    ...
    _hasActiveEditingFields() {
        return this.visiblePages.some(p => {
            var pageEditFields = p.editingFields;
    ...
    showActions() {
        if (this.zoomUpdating || this.interactionDisabled)
            return;
        var searchVisible = this.searchPanelVisible;
        if (!searchVisible) {
