...
module.exports = DevExpress.Analytics.Serializer.Native;
...
__webpack_require__.d(queryBuilder_widgets_internal_namespaceObject, "expressionFunctions", function() { return expressionFunctions; });
...
const metadata_name = { propertyName: 'name', modelName: '@Name', displayName: 'Name', editor: external_DevExpress_Analytics_Widgets_["editorTemplates"].getEditor('text'), validationRules: external_DevExpress_Analytics_Internal_["nameValidationRules"] };
const metadata_text = { propertyName: 'text', modelName: '@Text', displayName: 'Text', editor: external_DevExpress_Analytics_Widgets_["editorTemplates"].getEditor('text') };
const metadata_size = { propertyName: 'size', modelName: '@Size', defaultVal: '100,50', from: external_DevExpress_Analytics_Elements_["Size"].fromString, displayName: 'Size', editor: external_DevExpress_Analytics_Widgets_["editorTemplates"].getEditor('objecteditor') };
const metadata_location = { propertyName: 'location', modelName: '@Location', from: external_DevExpress_Analytics_Elements_["Point"].fromString, displayName: 'Location', editor: external_DevExpress_Analytics_Widgets_["editorTemplates"].getEditor('objecteditor') };
const sizeLocation = [metadata_size, metadata_location];
...
const diagramControlsFactory = new external_DevExpress_Analytics_Utils_["ControlsFactory"]();
...
const diagramElementSerializationInfo = [metadata_size, metadata_location, metadata_name, metadata_text, { propertyName: 'type', modelName: '@Type' }];
...
class diagramModel_DiagramViewModel extends diagramElementBaseViewModel_DiagramElementBaseViewModel {
    constructor(diagramSource) {
        const serializer = new external_DevExpress_Analytics_Utils_["ModelSerializer"]();
...
const margins = { propertyName: 'margins', modelName: '@Margins', from: external_DevExpress_Analytics_Elements_["Margins"].fromString, displayName: 'Margins' };
const pageWidth = { propertyName: 'pageWidth', modelName: '@PageWidth', defaultVal: 850, from: external_DevExpress_Analytics_Utils_["floatFromModel"], displayName: 'Page Width', editor: external_DevExpress_Analytics_Widgets_["editorTemplates"].getEditor('numeric') };
const pageHeight = { propertyName: 'pageHeight', modelName: '@PageHeight', defaultVal: 1250, from: external_DevExpress_Analytics_Utils_["floatFromModel"], displayName: 'Page Height', editor: external_DevExpress_Analytics_Widgets_["editorTemplates"].getEditor('numeric') };
    ...
    constructor(control, parent, serializer) {
        ...
        this._disposables.push(external_ko_["computed"](() => {
            if (!this.freezeRoute()) {
                ...
                const startPoint = new external_DevExpress_Analytics_Elements_["Point"](this.startPoint().location.x(), this.startPoint().location.y()), endPoint = new external_DevExpress_Analytics_Elements_["Point"](this.endPoint().location.x(), this.endPoint().location.y());
                ...
                let baseX = Math.min(startPoint.x(), endPoint.x()), baseY = Math.min(startPoint.y(), endPoint.y()), width = Math.abs(startPoint.x() - endPoint.x()), height = Math.abs(startPoint.y() - endPoint.y());
...
function createDiagramDesigner(element, diagramSource, localization, rtl) {
    ...
    registerControls();
    const diagram = external_ko_["pureComputed"](() => { return new diagramModel_DiagramViewModel(diagramSource()); }), surface = external_ko_["pureComputed"](() => {
        const surface = new diagramSurface_DiagramSurface(diagram());
        return surface;
    });
    const designerModel = Object(external_DevExpress_Analytics_Internal_["createDesigner"])(diagram, surface, diagramControlsFactory, initializer_groups, undefined, undefined, rtl);
    ...
    const updateSurfaceContentSize_ = Object(external_DevExpress_Analytics_Internal_["updateSurfaceContentSize"])(designerModel.surfaceSize, element);
    const onResize = () => {
...
external_ko_["bindingHandlers"]['routeLineDraggable'] = {
    init: (element, valueAccessor) => {
        ...
        const values = valueAccessor(), options = Object(external_DevExpress_Analytics_Internal_["extend"])({ snap: '.dxrd-drag-snap-line', snapTolerance: external_DevExpress_Analytics_Internal_["SnapLinesHelper"].snapTolerance }, external_ko_["unwrap"](values), {
...
class dbSchema_DBSchema {
    constructor(model) {
        const tables = deserializeToCollection(model['Tables'], (tableModel) => new dbTable_DBTable(tableModel));
        tables.sort((a, b) => { return a.name.localeCompare(b.name); });
        const views = deserializeToCollection(model['Views'], (tableModel) => new dbTable_DBTable(tableModel));
...
function getDBSchemaCallback({ requestWrapper, connection, tables, getTables, getViews }) {
    const deferred = external_jQuery_["Deferred"]();
...
const HandlerUri = Object(external_DevExpress_Analytics_Internal_["createGlobalModuleVariableFunc"])('DXQB.axd');
    ...
    getDbTable(tableName, fullPath = '') {
        ...
        if (!this._tables[tableName]) {
            ...
            schemaRequest.call(this).done((dbSchema) => {
                const table = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(dbSchema.tables, table => table.name === tableName);
                if (!table) {
                    deferred.reject();
...
class jsonSchemaNode_JsonNode {
    ...
    static toJsonNode(value, serializer, refs, recoursive = true) {
        ...
        const nodes = recoursive ? jsonSchemaNode_JsonNode.toJsonNodes(value.nodes, serializer, refs) : [];
        if (nodes.length > 0)
            obj['Node'] = nodes;
        return obj;
    }
...
class jsonSchemaNode_JsonSchemaNode extends jsonSchemaNode_JsonNode {
    ...
    static toJson(value, serializer, refs) {
        if (!value)
            return {};
        const obj = { Node: jsonSchemaNode_JsonNode.toJsonNode(value, serializer, refs, false) };
...
class jsonSchemaNode_JsonSchemaRootNode extends jsonSchemaNode_JsonNode {
    ...
    _fillRootElementList(node = this.nodes[0], currentPath = { fullPath: 'root', path: '', pathParts: ['root'] }, allowObjectRootElements) {
        if (!node)
            return this._rootElementList;
        const nodeType = external_ko_["unwrap"](node.nodeType);
...
const _getJsonSchemaCallback = (requestWrapper, jsonDataSource, parameters = []) => {
    ...
    requestWrapper.getJsonSchema(jsonDataSource, parameters)
        ...
        try {
            ...
            const jsonSchemaModel = new jsonSchemaNode_JsonSchemaRootNode(jsonSchema);
...
class jsonSchemaProvider_JsonSchemaProvider extends external_DevExpress_Analytics_Utils_["Disposable"] {
    constructor(jsonDataSource, _requestWrapper = new requestwrapper_RequestWrapper()) {
        ...
        this.getItems = (pathRequest) => {
            const getItemsDeferred = external_jQuery_["Deferred"]();
            const loadSchemaPromise = !this._jsonSchema ? this.getJsonSchema() : external_jQuery_["Deferred"]().resolve(this._jsonSchema).promise();
            loadSchemaPromise
                .done((jsonSchema) => {
...
class jsonDataSource_JsonDataSource extends external_DevExpress_Analytics_Utils_["Disposable"] {
    ...
    clone(_serializer) {
        const serializer = _serializer || new external_DevExpress_Analytics_Utils_["ModelSerializer"]();
...
const parameterValueSerializationsInfo = { propertyName: 'value', displayName: 'Value', localizationId: 'DevExpress.DataAccess.Parameter.Value', editor: external_DevExpress_Analytics_Widgets_["editorTemplates"].getEditor('text') };
const dsParameterName = { propertyName: 'name', displayName: 'Name', localizationId: 'DevExpress.DataAccess.Parameter.Name', validationRules: dsParameterNameValidationRules, editor: external_DevExpress_Analytics_Widgets_["editorTemplates"].getEditor('text') };
...
function storedProcParameterSerializationsInfo(type) {
    const copyParamType = Object(external_DevExpress_Analytics_Internal_["extend"])(true, {}, dsParameterType);
    ...
    const copyResultType = Object(external_DevExpress_Analytics_Internal_["extend"])(true, {}, dsExpressionResultType);
...
const expressionFunctions = ((addins) => Object(external_DevExpress_Analytics_Widgets_Internal_["combineFunctionDisplay"])(addins))({
    'String': {
...
class dataSourceParameter_DataSourceParameter extends external_DevExpress_Analytics_Utils_["Disposable"] {
    ...
    static _getTypeValue(typeName, resultType = null) {
        const result = dataSourceParameter_DataSourceParameter._typeValues.filter((type) => { return type.name === typeName; });
        if (result.length > 0) {
    ...
    get specifics() {
        const realTypeName = (Object(external_DevExpress_Analytics_Internal_["IsDataAccessExpression"])(this.type.peek()))
            ? this.resultType()
            : this.type();
        const result = dataSourceParameter_DataSourceParameter._typeValues.filter((type) => { return type.name === realTypeName; });
        if (result.length > 0)
    ...
    getInfo() {
        if (this.type) {
            const info = Object(external_DevExpress_Analytics_Internal_["extend"])(true, [], this._serializationsInfo);
...
const editorTemplates = new external_DevExpress_Analytics_Widgets_["EditorTemplates"]();
...
const metadata_alias = { propertyName: 'alias', modelName: '@Alias', displayName: 'Alias', localizationId: 'DataAccessUIStringId.QueryBuilderColumns_Alias', defaultVal: '', editor: editorTemplates.getEditor('text') };
...
const query_builder_metadata_size = { propertyName: 'size', modelName: '@Size', defaultVal: '100,125', from: external_DevExpress_Analytics_Elements_["Size"].fromString };
const query_builder_metadata_location = { propertyName: 'location', modelName: '@Location', from: external_DevExpress_Analytics_Elements_["Point"].fromString };
const metadata_sizeLocation = [query_builder_metadata_size, query_builder_metadata_location];
...
const controlsFactory = new external_DevExpress_Analytics_Utils_["ControlsFactory"]();
...
class subNode_SubNode {
    ...
    static deserializeRelationModel(subNodeQuery, relation) {
        ...
        relation.conditions().forEach(condition => {
            ...
            const operator = external_DevExpress_Analytics_Criteria_Utils_["operatorTokens"][condition.operator()];
            const conditionString = firstCondition + ' ' + operator + ' ' + secondCondition;
            conditionStrings.push(conditionString);
        });
        const subNode = new subNode_SubNode({ '@JoinType': relation.joinType(), '@Condition': conditionStrings.join(' And ') });
    ...
    _conditionBinary(operator) {
        ...
        const _operator = external_DevExpress_Analytics_Criteria_["BinaryOperatorType"][operator.operatorType];
        return this._createCondition(rightPath.split('.')[1], leftPath.split('.')[1], _operator);
    }
    createRelationModel(query) {
        const result = external_DevExpress_Analytics_Criteria_Utils_["CriteriaOperatorStateMachine"].parse(this.condition());
        const _operands = [];
...
class federatedQueriesContainer_FederatedQueriesContainer extends external_DevExpress_Analytics_Utils_["Disposable"] {
    ...
    removeSource(sourceName) {
        const source = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(this.sources(), source => source.sourceName() === sourceName);
    ...
    addSelectQuery(queryPath, columnName) {
        ...
        const expression = new federatedQueryExpression_FederatedQueryExpression({
            '@NodeAlias': queryName,
            '@Name': columnName,
...
class federationMasterDetailRelation_FederationMasterDetailRelation extends masterDetailRelation_MasterDetailRelation {
    static create(_relation) {
        const serializer = new external_DevExpress_Analytics_Utils_["ModelSerializer"]();
        const relation = new federationMasterDetailRelation_FederationMasterDetailRelation(serializer.serialize(_relation), serializer);
...
class federationSerializableModel_SerializableDataFederationDataSource extends external_DevExpress_Analytics_Utils_["Disposable"] {
    ...
    _collectDependentDataSources(item, resultArray) {
        ...
        const itemName = external_ko_["unwrap"](item.name);
    ...
    serialize() {
        const clonedModel = new federationSerializableModel_SerializableDataFederationDataSource(null);
...
const AutoQueryPreload = Object(external_DevExpress_Analytics_Internal_["createGlobalModuleVariableFunc"])(true);
...
const _defaultRestoreSqlDataSourceFromState = (state, requestWrapper, dataSourceId) => {
    const wrapper = new _sqlDataSourceWrapper_SqlDataSourceWrapper(state.sqlDataSourceJSON, state.queryName, requestWrapper);
...
const _defaultRestoreFederationDataSourceFromState = (state, usedDataSources, dataSourceId) => {
    const dataSource = new federationDataSource_FederationDataSource(JSON.parse(state.federationDataSourceJSON), usedDataSources);
    if (dataSourceId) {
...
const _defaultRestoreJsonDataSourceFromState = (state, requestWrapper, dataSourceId) => {
    const jsonDataSource = new jsonDataSource_JsonDataSource({
...
function _restoreObjectDataSourceFromState(state, requestWrapper, dataSourceId) {
    const objectDataSource = new objectDataSource_ObjectDataSource();
    ...
    initialize(state) {
        const selectConnection = (connections) => {
            if (connections.length === 1) {
                ...
                const selectedString = Object(external_DevExpress_Analytics_Internal_["getFirstItemByPropertyValue"])(connections, 'name', state.name) || connections[0];
...
class _chooseObjectParameters_ObjectDataSourceParameterProperty extends external_DevExpress_Analytics_Utils_["Disposable"] {
    constructor(_parameter, _itemsProvider) {
        ...
        const typeValue = dataSourceParameter_DataSourceParameter._getTypeValue(this._parameter.resultType);
...
class _chooseObjectParameters_ObjectDataSourceParameterGrid extends external_DevExpress_Analytics_Widgets_["ObjectProperties"] {
    constructor(x) {
        ...
        this.createEditorAddOn = (_editor) => {
            const editor = Object(external_DevExpress_Analytics_Widgets_["unwrapEditor"])(_editor);
...
class _chooseObjectParameters_ObjectDataSourceParametersModel extends external_DevExpress_Analytics_Utils_["Disposable"] {
    constructor(parametersMethod, itemsProvider) {
        ...
        this._grids = parametersMethod.parameters.map(x => {
            const param = new _chooseObjectParameters_ObjectDataSourceParameterProperty(x, itemsProvider);
            const grid = new _chooseObjectParameters_ObjectDataSourceParameterGrid(param);
...
class _objectSchemaProvider_ObjectTypeDescriptions {
    constructor(model) {
        const types = deserializeToCollection(model, (key) => new objectSchema_ObjectType(key));
...
function getObjectTypeDescriptionsCallback(requestWrapper, context) {
    ...
    requestWrapper.getObjectTypeDescriptions(context)
        .done(data => {
        try {
            const objectTypeDescriptionsModel = new _objectSchemaProvider_ObjectTypeDescriptions(data.objectDataSourceInfoJson);
...
// EXTERNAL MODULE: external "DevExpress.Analytics.Serializer.Native"
var external_DevExpress_Analytics_Serializer_Native_ = __webpack_require__(10);
...
class _utils_CustomQueryTreeListItem extends external_DevExpress_Analytics_Widgets_Internal_["TreeListItemViewModel"] {
    ...
    createViewModel() {
        return Object(external_DevExpress_Analytics_Serializer_Native_["createViewModelGenerator"])(super.createViewModel())
    ...
    onPropertyChanged(args) {
        ...
        if (args.propertyName === 'queryName') {
            let propertyChangedEventArgs = args;
...
__decorate([
    Object(external_DevExpress_Analytics_Serializer_Native_["mutable"])('')
], _utils_CustomQueryTreeListItem.prototype, "queryName", void 0);
class _utils_MultiQueryTreeListItemFactory extends external_DevExpress_Analytics_Widgets_Internal_["KoTreeListItemFactory"] {
    createItem(options, path, onItemsVisibilityChanged, rtl, resolver) {
        if (path[0] === 'queries') {
            return new _utils_CustomQueryTreeListItem(Object(external_DevExpress_Analytics_Widgets_Internal_["wrapTreeListOptionsWithKo"])(options), path, onItemsVisibilityChanged, rtl, resolver);
        }
...
function getLocalizedValidationErrorMessage(emptyValueErrorMessage, localizedPropertyName, subProperty) {
    const requiredMessageSuffix = emptyValueErrorMessage || Object(external_DevExpress_Analytics_Utils_["getLocalization"])('The value cannot be empty', 'AnalyticsCoreStringId.ParametersPanel_DateTimeValueValidationError');
    if (!localizedPropertyName)
        return requiredMessageSuffix;
    let propertyNamesPrefix = !subProperty ? localizedPropertyName : Object(external_DevExpress_Analytics_Internal_["formatUnicorn"])('{0}. {1}', localizedPropertyName, subProperty);
...
class _jsonSourceSettings_JsonDataSourceJsonSourcePageUriSettings extends _jsonSourceSettings_JsonDataSourceJsonSourcePageSettingsBase {
    ...
    _getPatchedParameter(parameter) {
        parameter.nameValidationRules = [{
                type: 'required',
                get message() {
                    return getLocalizedValidationErrorMessage(null, Object(external_DevExpress_Analytics_Utils_["getLocalization"])('Parameters', 'AnalyticsCoreStringId.QueryBuilder_Parameters'), this._collectionItemNamePlaceholder);
                }
            }];
    ...
    _validateUriSource() {
        const defaultValidationErrorMessage = Object(external_DevExpress_Analytics_Utils_["getLocalization"])('Invalid URI.', 'AnalyticsCoreStringId.ReportDesigner_Wizard_JsonSource_UriValidationError');
        const endpointUriDisplayName = Object(external_DevExpress_Analytics_Utils_["getLocalization"])('Web Service Endpoint (URI):', 'DataAccessUIStringId.WizardPageChooseJsonSource_URI');
        this._isUriValid(false);
        if (this._lastValidateDeferred) {
            this._lastValidateDeferred.reject();
        }
        const resultDeferred = external_jQuery_["Deferred"]();
    ...
    _getParametersInfo() {
        const parameters = {
            ...
            array: true,
            addHandler: () => {
                const newName = Object(external_DevExpress_Analytics_Internal_["getUniqueNameForNamedObjectsArray"])(this.parameters(), 'parameter');
                return this._getPatchedParameter(jsonParameter_JsonParameter.from({ '@Name': newName }));
            },
...
class chooseJsonSourcePage_ChooseJsonSourcePage extends wizardPageBase_WizardPageBase {
    constructor(_requestWrapper = new requestwrapper_RequestWrapper(), itemsProvider) {
        ...
        const selectedSource = external_ko_["observable"]();
    ...
    commit() {
        ...
        const serialized = new external_DevExpress_Analytics_Utils_["ModelSerializer"]().serialize(jsonDataSource);
        return external_jQuery_["Deferred"]().resolve({
...
class specifyJsonConnectionPage_SpecifyJsonConnectionPage extends chooseJsonConnectionPage_ChooseJsonConnectionPage {
    ...
    commit() {
        const deffered = external_jQuery_["Deferred"]();
        let _promise;
        if (this._createNew()) {
...
class _treeNodeItemsProvider_TreeNodeItemsProvider extends external_DevExpress_Analytics_Utils_["Disposable"] {
    constructor(fieldListProvider, rootItems, generateTreeNode, generateTreeLeafNode) {
        ...
        this.getItems = (pathRequest, collectChilds = false) => {
            const result = external_jQuery_["Deferred"]();
    ...
    _createTreePart(pathParts, deferred = external_jQuery_["Deferred"](), checkedPath) {
        ...
        if (!checkedPath) {
            ...
            const request = new external_DevExpress_Analytics_Utils_["PathRequest"](newParentPath.join('.'), newParentPath);
...
class chooseJsonSchemaPage_ChooseJsonSchemaPage extends wizardPageBase_WizardPageBase {
    constructor(_requestWrapper = new requestwrapper_RequestWrapper(), _allowObjectRootElements = true, _callbacks) {
        ...
        this._createTreeNode = (item, isChecked, path) => {
            const node = new _treeListNode_DataMemberTreeNode(item.name, item.displayName, item.specifics, isChecked, path);
            this._disposables.push(node.checked.subscribe(() => this._onChange()));
            return node;
        };
        this._createLeafTreeNode = (item, isChecked, path) => {
            const node = new _treeListNode_FieldTreeNode(item.name, item.displayName, item.specifics, isChecked, path);
            this._disposables.push(node.checked.subscribe(() => this._onChange()));
            return node;
        };
        ...
        const fieldListProvider = new external_DevExpress_Analytics_Internal_["FieldListProvider"](this._createFieldListCallback(), this._rootItems);
    ...
    _createFieldListCallback() {
        return (pathRequest) => {
            const parentNode = new jsonSchemaNode_JsonNode({});
    ...
    commit() {
        ...
        const serialized = new external_DevExpress_Analytics_Utils_["ModelSerializer"]().serialize(this._dataSource);
        return external_jQuery_["Deferred"]().resolve({
...
class _queryBuilderPopup_QueryBuilderPopupBase extends external_DevExpress_Analytics_Utils_["Disposable"] {
    ...
    popupViewModel(element) {
        const $container = Object(external_DevExpress_Analytics_Internal_["getParentContainer"])(element);
        return {
...
_queryBuilderPopup_QueryBuilderPopupBase.customizeQueryBuilderActions = (sender, args) => {
    ...
    const del = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(actions, action => action.text === 'Delete'), undo = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(actions, action => action.text === 'Undo'), redo = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(actions, action => action.text === 'Redo');
...
class _selectQuerySqlTextProvider_SelectQuerySqlTextProvider {
    ...
    getQuerySqlText(newQuery) {
        const queryJSON = JSON.stringify({ 'Query': new external_DevExpress_Analytics_Utils_["ModelSerializer"]().serialize(newQuery) });
        return this._selectStatementCallback(this._connection(), queryJSON)
            .fail((data) => {
            const error = Object(external_DevExpress_Analytics_Internal_["getErrorMessage"])(data);
...
if (!!external_window_ace_default.a) {
    external_window_ace_default.a.define('ace/mode/sql_highlight_rules', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text_highlight_rules', 'ace/mode/doc_comment_highlight_rules'], function (require, exports, module) {
        'use strict';
        const oop = external_window_ace_default.a.require('ace/lib/oop');
        const DocCommentHighlightRules = external_window_ace_default.a.require('ace/mode/doc_comment_highlight_rules').DocCommentHighlightRules;
        const TextHighlightRules = external_window_ace_default.a.require('ace/mode/text_highlight_rules').TextHighlightRules;
    ...
    external_window_ace_default.a.define('ace/mode/sql', ['require', 'exports', 'module', 'ace/lib/oop', 'ace/mode/text', 'ace/mode/sql_highlight_rules', 'ace/range'], function (require, exports, module) {
        'use strict';
        const oop = external_window_ace_default.a.require('ace/lib/oop');
        const TextMode = external_window_ace_default.a.require('ace/mode/text').Mode;
        const SqlHighlightRules = external_window_ace_default.a.require('ace/mode/sql_highlight_rules').SqlHighlightRules;
        const Range = external_window_ace_default.a.require('ace/range').Range;
        const Mode = function () {
        ...
        this.sqlString = external_ko_["pureComputed"]({
            ...
            write: (val) => {
                if (this._query().type() !== SqlQueryType.customSqlQuery) {
                    const customQuery = new customSqlQuery_CustomSqlQuery({ '@Name': this._query().name() }, this._query().parent);
...
class _storedProceduresQueryControl_StoredProceduresQueryControl extends external_DevExpress_Analytics_Utils_["Disposable"] {
    ...
    getQuery() {
        if (!this._selectedProcedure)
            return null;
        const newQuery = new storedProcQuery_StoredProcQuery({ '@Name': this._query.name() || this._selectedProcedure.name, 'ProcName': this._selectedProcedure.name }, this._query.parent);
        ...
        this.getItems = (pathRequest) => {
            ...
            if (!pathRequest.fullPath) {
                ...
                dbSchemaProvider.getDbStoredProcedures().done((storedProcedures) => {
                    if (this._procedures.children().length === 0) {
                        const procedures = storedProcedures.map(proc => {
                            const node = new TreeLeafNode(proc.name, _storedProceduresQueryControl_StoredProceduresQueryControl.generateStoredProcedureDisplayName(proc), 'procedure', this._procedures.checked.peek(), proc.arguments, afterCheckToggled);
                ...
                const queries = customQueries().map(query => {
                    ...
                    if (currentQuery)
                        return currentQuery;
                    const queryNode = new _treeListNode_TreeQueryNode(name, name, 'query', !!currentQuery && currentQuery.checked(), query.parameters, this._callBack, afterCheckToggled, query);
...
class stateManager_StateManager {
    ...
    resetPageState(pageId) {
        const defaultState = Object(external_DevExpress_Analytics_Internal_["extend"])(true, {}, this._getPageState(pageId, this.defaultState));
...
class pageIterator_PageIterator extends external_DevExpress_Analytics_Utils_["Disposable"] {
    ...
    _getNextNewPage(nextPageId) {
        ...
        const newPage = new _wrappedWizardPage_WrappedWizardPage(nextPageId, pageMetadata.create(), pageMetadata.template, pageMetadata.description);
    ...
    _getStartPage(pageId) {
        ...
        const startPage = new _wrappedWizardPage_WrappedWizardPage(pageId, pageMetadata.create(), pageMetadata.template, pageMetadata.description);
...
class popupWizard_PopupWizard extends wizard_BaseWizard {
    ...
    static _getLoadPanelViewModel(element, observableVisible) {
        const $container = external_jQuery_["fn"].constructor(element).closest('.dxrd-wizard-content');
        return {
            animation: {
...
class multiQueryConfigurePage_MultiQueryConfigurePage extends wizardPageBase_WizardPageBase {
    constructor(_options) {
        ...
        this._showQbCallBack = (name = null, isCustomQuery = false) => {
            if (name !== null) {
                const query = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(this._customQueries(), item => name === (item.name() || item.generateName()));
                ...
                if (isCustomQuery) {
                    ...
                    const queryNew = new tableQuery_TableQuery({ '@Name': null }, this._dataSource());
    ...
    _addQueryFromStoredProcedures(elements, dataSource) {
        for (let i = 0; i < elements.children().length; i++) {
            ...
            if (procedure.checked()) {
                const newQuery = new storedProcQuery_StoredProcQuery({ '@Name': procedure.name, 'ProcName': procedure.name }, dataSource);
    ...
    _addQueryFromCustomQueries(elements, queries, allQueries) {
        for (let i = 0; i < elements.children().length; i++) {
            ...
            const query = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(queries.peek(), item => queryNode.name === (item.name() || item.generateName()));
    ...
    commit() {
        ...
        const customQueries = this._sqlDataSourceWrapper.sqlDataSource.queries().filter(x => checkedCustomQueries.some(queryName => queryName === x.name()) || x instanceof storedProcQuery_StoredProcQuery);
    ...
    initialize(state) {
        ...
        customQueriesPromise.done((queries) => {
            ...
            this._selectedPath('');
            const itemsProvider = new _dbSchemaItemsProvider_DBSchemaItemsProvider(this._dataSource().dbSchemaProvider, this._customQueries, this._showQbCallBack, this.disableCustomSql, this._getItemsAfterCheck);
            ...
            const searchOptions = new external_DevExpress_Analytics_Widgets_Internal_["TreeListSearchOptions"]();
...
class multiQueryConfigureParametersPage_MultiQueryConfigureParametersPage extends wizardPageBase_WizardPageBase {
    constructor(parametersConverter = {
        ...
        this._createNewParameter = (queryName, parameters) => {
            const newParameter = new dataSourceParameter_DataSourceParameter({
            ...
            itemsProvider: {
                getItems: pathRequest => {
                    ...
                    if (!pathRequest.fullPath) {
                        ...
                        const parent = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(this._rootItems(), item => item.name === pathRequest.ref);
    ...
    commit() {
        this._rootItems().forEach(item => {
            const customQuery = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(this._sqlDataSourceWrapper.customQueries, (query) => query.name() === item.query().name());
...
class _keyColumnSurface_KeyColumnSurface {
    constructor(column, queryName, _isMaster = false) {
        ...
        this._setColumn = (resultColumn) => {
            this.column(external_ko_["unwrap"](resultColumn.name));
        };
    }
}
...
class _masterQuerySurface_MasterQuerySurface {
    constructor(masterQueryName, relations) {
        ...
        this.create = (detailQueryItem) => {
            const newRelation = new masterDetailRelation_MasterDetailRelation({ '@Master': this.queryName, '@Detail': detailQueryItem.name });
...
class _masterDetailEditor_MasterDetailEditor extends external_DevExpress_Analytics_Widgets_Internal_["PopupEditorBase"] {
    constructor(relations, resultSet, saveCallBack) {
        ...
        this.setColumn = (target) => {
            const table = Object(external_DevExpress_Analytics_Internal_["getFirstItemByPropertyValue"])(resultSet.tables(), 'tableName', target.queryName);
            return {
            ...
            try {
                ...
                Object(external_DevExpress_Analytics_Criteria_Utils_["criteriaForEach"])(this._criteria, (operand) => {
                    if (operand instanceof external_DevExpress_Analytics_Criteria_["OperandProperty"]) {
                        const dependedTable = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(query.tables(), table => operand.propertyName.indexOf(table.actualName() + '.') === 0);
...
class allColumnsModel_AllColumnsViewModel extends queryElementModel_QueryElementBaseViewModel {
    constructor(parent, serializer) {
        ...
        const targetColumn = external_ko_["pureComputed"](() => {
...
allColumnsModel_AllColumnsViewModel.DisplayName = () => {
    const name = Object(external_DevExpress_Analytics_Utils_["getLocalization"])('(All Columns)', 'DataAccessStringId.QueryBuilder_AllColumns');
...
class _columnExpressionCollectionHelper_ColumnExpressionCollectionHelper {
    ...
    static createNew(query, collection, table, column) {
        const newItem = new columnExpression_ColumnExpression({ '@Table': table, '@Name': column, '@ItemType': 'Column' }, query);
    ...
    constructor(query, objectFilter) {
        this.getItems = (pathRequest) => {
            ...
            if (pathRequest.fullPath === '') {
                ...
                const table = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(query().tables(), table => table.actualName() === pathRequest.fullPath);
        ...
        this.getColumnInfo = (propertyName) => {
            if (propertyName) {
                const table = Object(external_DevExpress_Analytics_Internal_["find"])(query().tables(), t => propertyName.indexOf(t.actualName() + '.') === 0);
                if (table) {
                    const column = Object(external_DevExpress_Analytics_Internal_["find"])(objectFilter.filterColumns(table.columns() || []), c => propertyName === (table.actualName() + '.' + objectFilter.getColumnName(c)));
...
class _groupFilterEditorSerializer_GroupFilterEditorSerializer extends external_DevExpress_Analytics_Widgets_Internal_["FilterEditorSerializer"] {
    ...
    deserialize(stringCriteria) {
        const operand = external_DevExpress_Analytics_Criteria_Utils_["CriteriaOperatorStateMachine"].parse(stringCriteria);
        if (operand) {
...
var external_DevExpress_Analytics_Widgets_Filtering_ = __webpack_require__(11);
...
class _operandParameterQBSurface_OperandParameterQBSurface extends external_DevExpress_Analytics_Widgets_Filtering_["OperandParameterSurface"] {
    ...
    _createParameter(name, dataType) {
        if (name !== '' && name !== _operandParameterQBSurface_OperandParameterQBSurface.defaultDisplay() && this.helper.parameters().filter((parameter) => { return parameter.name() === name; }).length === 0) {
            const parameter = new parameterModel_ParameterViewModel({ '@Name': name, '@Type': dataType });
...
class _operandPropertyQBSurface_OperandPropertyQBSurface extends external_DevExpress_Analytics_Widgets_Filtering_["OperandPropertySurface"] {
    ...
    static updateSpecifics(propertySurface) {
        const fieldList = external_ko_["unwrap"](propertySurface.fieldListProvider);
        if (fieldList && fieldList.getColumnInfo) {
            ...
            if (item) {
                ...
                const _fieldsOptions = external_ko_["unwrap"](propertySurface.fieldsOptions);
                if (_fieldsOptions)
...
let QBFilterEditorHelperDefault = _qbFilterEditorHelper_QBFilterEditorHelper;
...
class columnModel_ColumnViewModel extends queryElementModel_QueryElementBaseViewModel {
    constructor(model, dbColumn, parent, serializer) {
        ...
        const query = parent.parentModel();
        const targetColumn = external_ko_["pureComputed"](() => _columnExpressionCollectionHelper_ColumnExpressionCollectionHelper.find(query.columns, parent.actualName(), this.name()));
        ...
        const orderByItem = external_ko_["pureComputed"](() => _columnExpressionCollectionHelper_ColumnExpressionCollectionHelper.find(query.sorting, parent.actualName(), this.name()));
        ...
        const groupByItem = external_ko_["computed"](() => _columnExpressionCollectionHelper_ColumnExpressionCollectionHelper.find(query.grouping, parent.actualName(), this.name()));
...
class tableModel_TableViewModel extends queryElementModel_QueryElementBaseViewModel {
    ...
    getColumnConnectionPoints(column) {
        const y = external_ko_["pureComputed"]({
            read: () => {
...
class queryModel_QueryViewModelBase extends queryElementModel_QueryElementBaseViewModel {
    constructor(querySource, dbSchemaProvider, parametersMode = ParametersMode.ReadWrite, beforeSaveCallback, serializer) {
        ...
        const _pageWidth = external_ko_["observable"](0);
    ...
    cerateJoinCondition(parentColumn, nestedColumn) {
        ...
        let relation = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(this.relations(), (relation) => {
        ...
        let joinCondition = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(relation.conditions(), (condition) => {
...
class queryModel_QueryViewModel extends queryModel_QueryViewModelBase {
    constructor(querySource, dbSchemaProvider, parametersMode = ParametersMode.ReadWrite, beforeSaveCallback, serializer) {
        ...
        const parameters = Object(external_DevExpress_Analytics_Utils_["deserializeArray"])(querySource['Parameters'], item => new parameterModel_ParameterViewModel(item, serializer));
        if (parametersMode === ParametersMode.ReadWrite) {
            this.parameters = parameters;
        ...
        this.dbSchemaProvider.getDbSchema().done((dbSchema) => {
            ...
            if (dbSourceTable) {
                dbSourceTable.foreignKeys.forEach((fk) => {
                    const pkTable = Object(external_DevExpress_Analytics_Internal_["getFirstItemByPropertyValue"])(this.tables.peek(), 'name', fk.primaryKeyTable);
                    if (pkTable) {
                        for (let i = 0; i < fk.primaryKeyColumns.length; i++) {
                            const column1 = Object(external_DevExpress_Analytics_Internal_["getFirstItemByPropertyValue"])(sourceTable.columns(), 'name', fk.columns[i]);
                            const column2 = Object(external_DevExpress_Analytics_Internal_["getFirstItemByPropertyValue"])(pkTable.columns(), 'name', fk.primaryKeyColumns[i]);
                            if (column1 && column2) {
                ...
                if (dbTable) {
                    dbTable.foreignKeys.forEach((fk) => {
                        if (fk.primaryKeyTable === sourceTable.name()) {
                            for (let i = 0; i < fk.primaryKeyColumns.length; i++) {
                                const column1 = Object(external_DevExpress_Analytics_Internal_["getFirstItemByPropertyValue"])(sourceTable.columns(), 'name', fk.primaryKeyColumns[i]);
                                const column2 = Object(external_DevExpress_Analytics_Internal_["getFirstItemByPropertyValue"])(table.columns(), 'name', fk.columns[i]);
                                if (column1 && column2) {
                                    this.cerateJoinCondition(column2, column1);
                                }
                            }
...
class _federationQueryModel_FederationQueryViewModel extends queryModel_QueryViewModelBase {
    ...
    serialize(includeRootTag) {
        const serializer = this.serializer || new external_DevExpress_Analytics_Utils_["ModelSerializer"]();
        ...
        this.subNodes(_tables.map(table => {
            let relation = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(_relations, relation => relation.nestedTable() === table);
            if (!relation)
...
class _federationQueryBuilderPopupBase_FederationQueryBuilderPopupBase extends external_DevExpress_Analytics_Widgets_Internal_["PopupEditorBase"] {
    constructor(onSaveCallback, _dataSource, rtl = false, onCloseCallback) {
        ...
        const treeListController = new _federationUnionQueryBuilderTreeListController_FederatedUnionQueryBuilderTreeListController(this.dragDropHandler, (item) => treeListController.isDraggable(item) && this.addDataMember(item));
        this.fieldListModel = {
...
class _federationTablesExpressionFieldListProvider_FederationTablesExpressionFieldListProvider {
    ...
    getItems(pathRequest) {
        ...
        if (pathRequest.path === '') {
            ...
            paths.shift();
            const table = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(this.tables(), table => table.actualName() === paths.join('.'));
            if (table) {
...
class _federationSelectQueryBuilderPopup_FederationSelectQueryBuilderPopup extends _federationQueryBuilderPopupBase_FederationQueryBuilderPopupBase {
    constructor(onSaveCallback, dataSource, rtl = false, onCloseCallback) {
        ...
        const cellTemplateMask = (container, options) => {
            ...
            const templateHtml = Object(external_DevExpress_Analytics_Widgets_["getTemplate"])('dxrd-querybuilder-column-combobox-masked'), $element = external_jQuery_["fn"].constructor(container).append(templateHtml);
            const model = {
        ...
        const cellTemplate = (container, options) => {
            let $container = external_jQuery_["fn"].constructor(container);
                    ...
                    validationRules: [this._aliasValidationRule, {
                            ...
                            validationCallback: (options) => {
                                if (options.value === '' && options.data && options.data.key) {
                                    const existedColumn = _columnExpressionCollectionHelper_ColumnExpressionCollectionHelper.findByName(this._getQuery().columns, options.data.key);
    ...
    _onContentReady(e) {
        if (!e.component._isFirstLoad) {
            ...
            const element = external_jQuery_["fn"].constructor(e.component.content())[0];
    ...
    _changeColumn(columnData, insertIndex) {
        if (columnData) {
            let query = _columnExpressionCollectionHelper_ColumnExpressionCollectionHelper.createNew(this._getQuery(), this._getQuery().columns, columnData.table, columnData.column);
    ...
    addDataMember(item, position) {
        ...
        const newControl = query.createChild(Object(external_DevExpress_Analytics_Internal_["extend"])({
            '@ControlType': 'FTable',
            '@Name': name
        }, controlsFactory.controlsMap['FTable'].defaultVal), undefined, item.path);
        const controlSurface = external_ko_["unwrap"](Object(external_DevExpress_Analytics_Internal_["findSurface"])(newControl));
        if (!position) {
            ...
            const _querySurface = external_ko_["unwrap"](Object(external_DevExpress_Analytics_Internal_["findSurface"])(this.designer()));
...
class _federationTransformQueryBuilderPopup_FederationTransformQueryBuilderPopup extends _federationQueryBuilderPopupBase_FederationQueryBuilderPopupBase {
    constructor(onSaveCallback, dataSource, rtl = false, onCloseCallback) {
        ...
        const transformResultSchemaProvider = new _federationTransformResultSchemaProvider_TransformResultSchemaProvider(this._dataSource.dbSchemaProvider, this.transformSources, this.currentPath);
    ...
    _aliasValidationCallback(alias, data) {
        const existedColumn = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(this.transformSources(), source => source.alias === alias);
...
class _federationUnionQueryBuilderPopup_FederationUnionQueryBuilderPopup extends _federationQueryBuilderPopupBase_FederationQueryBuilderPopupBase {
    constructor(onSaveCallback, dataSource, rtl = false, onCloseCallback) {
        ...
        const sources = external_ko_["pureComputed"](() => this.unionQuery() && this.unionQuery().queries().map(x => {
    ...
    _aliasValidationCallback(alias, data) {
        const existedColumn = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(this.columns(), source => source.alias === alias);
        if (existedColumn && existedColumn.key === data.key)
            return true;
        return !existedColumn;
    }
            ...
            if (pathRequest.fullPath === 'queries') {
                const queries = customQueries().map(query => {
                    ...
                    if (query instanceof unionQuery_UnionQuery) {
                        callback = external_ko_["observable"](Object(external_DevExpress_Analytics_Internal_["extend"])(this._callBack(), { showQbCallBack: callBacks.unionCallBack }));
                    }
                    else if (query instanceof transformQuery_TransformQuery) {
                        callback = external_ko_["observable"](Object(external_DevExpress_Analytics_Internal_["extend"])(this._callBack(), { showQbCallBack: callBacks.transformCallBack }));
                    }
                    else {
                        callback = external_ko_["observable"](Object(external_DevExpress_Analytics_Internal_["extend"])(this._callBack(), { showQbCallBack: callBacks.joinCallBack }));
                    }
                    const queryNode = new _treeListNode_TreeQueryNode(name, name, 'query', !!currentQuery && currentQuery.checked(), external_ko_["observableArray"]([]), callback, afterCheckToggled, query);
...
class federatedQueryConfigurePage_FederatedQueryConfigurePage extends wizardPageBase_WizardPageBase {
    constructor(_options) {
        ...
        this._setQueryChecked = (query) => {
            ...
            const queries = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(rootItems, (data) => data.name === 'queries');
        ...
        this._getItemsAfterCheck = (node) => {
            ...
            if (node.specifics == 'query') {
                const query = node['query'] || Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(this._customQueries(), (data) => (data.alias() || query.generateName()) === node.name);
    ...
    _wrapFieldListCallback(itemsCallback, useCache) {
        return (pathRequest) => {
            const _dataSource = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(this._dataSources(), x => {
    ...
    initialize(state) {
        const deferred = external_jQuery_["Deferred"]();
        const rootItems = external_ko_["observableArray"](external_jQuery_["map"](this._dataSources(), (item) => {
            return {
                name: item.id || item.ref,
                displayName: item.name,
                isList: true,
        ...
        const itemsProvider = new _federationTreeNodeProvider_FederationTreeNodeProvider(this._fieldListProvider, rootItems, this._queriesPopupHelper.callBacks, this._customQueries, this._getItemsAfterCheck);
        this._disposables.push(itemsProvider);
        this._itemsProvider(itemsProvider);
        this._fieldListModel({
            factory: this._createTreeListFactory(),
...
class federatedMasterDetailRelationshipsPage_FederatedMasterDetailRelationshipsPage extends masterDetailRelationshipsPageBase_MasterDetailRelationshipsPageBase {
    ...
    commit() {
        this.relationsSubscription.dispose();
        const relations = this._relations().map(x => federationMasterDetailRelation_FederationMasterDetailRelation.create(x));
...
class _wizardPageSectionIterator_WizardPageSectionIterator {
    ...
    getStartPage() {
        ...
        const page = new _wrappedWizardPageSection_WrappedWizardPageSection(startPageId, pageMetadata.create(), pageMetadata);
...
function _createDataSourceFullscreenWizard(dataSourceWizardOptions) {
    const factory = new fullscreenWizardPageFactory_FullscreenWizardPageFactory();
    const wizard = new fullscreenMultiQueryDataSourceWizard_FullscreenDataSourceWizard(factory, dataSourceWizardOptions);
...
class _manageFederatedQueriesEditor_ManageFederatedQueriesEditor extends external_DevExpress_Analytics_Widgets_Internal_["PopupEditorBase"] {
    ...
    save() {
        this.queriesStoreData().forEach(data => {
            const query = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(this._dataSource.queries(), query => query.alias() === data.id);
            query.sources().forEach(x => this._dataSource.addSource(x));
...
Object(external_DevExpress_Analytics_Widgets_Internal_["addToBindingsCache"])('attr: { points: routePointsSet, \'marker-end\': showArrow() ? \'url(#dxqb-arrow_end\' + connectorID() + \')\' : \'\', \'marker-start\': showRightArrow() ? \'url(#dxqb-arrow_start\' + connectorID() + \')\' : \'\' }', function ($context, $element) { return { 'attr': function () { return { 'points': $context.$data.routePointsSet, 'marker-end': $context.$data.showArrow() ? 'url(#dxqb-arrow_end' + $context.$data.connectorID() + ')' : '', 'marker-start': $context.$data.showRightArrow() ? 'url(#dxqb-arrow_start' + $context.$data.connectorID() + ')' : '' }; } }; });
...
Object(external_DevExpress_Analytics_Widgets_Internal_["addToBindingsCache"])('text: $root.getLocalization(value.name), attr: { title: $root.getLocalization(value.name) }', function ($context, $element) { return { 'text': function () { return $context.$root.getLocalization($context.$data.value.name); }, 'attr': function () { return { 'title': $context.$root.getLocalization($context.$data.value.name) }; } }; });
Object(external_DevExpress_Analytics_Widgets_Internal_["addToBindingsCache"])('template: { name: \'dx-propertieseditor\', data: getProperties({ level: 1 }) }', function ($context, $element) { return { 'template': function () { return { 'name': 'dx-propertieseditor', 'data': $context.$data.getProperties({ 'level': 1 }) }; } }; });
...
Object(external_DevExpress_Analytics_Widgets_Internal_["addToBindingsCache"])('foreach: getEditors()', function ($context, $element) { return { 'foreach': function () { return $context.$data.getEditors(); } }; });
...
Object(external_DevExpress_Analytics_Widgets_Internal_["addToBindingsCache"])('text: displayName, attr: { \'title\': displayName }', function ($context, $element) { return { 'text': function () { return $context.$data.displayName; }, 'attr': function () { return { 'title': $context.$data.displayName }; } }; });
...
Object(external_DevExpress_Analytics_Widgets_Internal_["addToBindingsCache"])('if: $data.value', function ($context, $element) { return { 'if': function () { return $context.$data.value; } }; });
...
Object(external_DevExpress_Analytics_Widgets_Internal_["addToBindingsCache"])('dxTextBox: { value: name, placeholder: $data.namePlaceholder() }, dxValidator: { validationRules: nameValidationRules || [] }', function ($context, $element) { return { 'dxTextBox': function () { return { 'value': $context.$data.name, 'placeholder': $context.$data.namePlaceholder() }; }, 'dxValidator': function () { return { 'validationRules': $context.$data.nameValidationRules || [] }; } }; });
...
Object(external_DevExpress_Analytics_Widgets_Internal_["addToBindingsCache"])('dxExpressionEditor: { options: _expression, fieldListProvider: itemsProvider, displayNameProvider: $root.displayNameProvider && $root.displayNameProvider() }, visible: $data.isExpression', function ($context, $element) { return { 'dxExpressionEditor': function () { return { 'options': $context.$data._expression, 'fieldListProvider': $context.$data.itemsProvider, 'displayNameProvider': $context.$root.displayNameProvider && $context.$root.displayNameProvider() }; }, 'visible': function () { return $context.$data.isExpression; } }; });
...
Object(external_DevExpress_Analytics_Widgets_Internal_["addToBindingsCache"])('dxTextBox: { value: queryName, onValueChanged: queryNameHasChanged }', function ($context, $element) { return { 'dxTextBox': function () { return { 'value': $context.$data.queryName, 'onValueChanged': $context.$data.queryNameHasChanged }; } }; });
...
Object(external_DevExpress_Analytics_Widgets_Internal_["addToBindingsCache"])('if: showIcon', function ($context, $element) { return { 'if': function () { return $context.$data.showIcon; } }; });
Object(external_DevExpress_Analytics_Widgets_Internal_["addToBindingsCache"])('ifnot: showIcon', function ($context, $element) { return { 'ifnot': function () { return $context.$data.showIcon; } }; });
...
Object(external_DevExpress_Analytics_Widgets_Internal_["addToBindingsCache"])('dxdAccordion: $data', function ($context, $element) { return { 'dxdAccordion': function () { return $context.$data; } }; });
...
Object(external_DevExpress_Analytics_Widgets_Internal_["addToBindingsCache"])('dxclick: select, css: { \'dxd-state-selected dxd-back-secondary\' : selected }', function ($context, $element) { return { 'dxclick': function () { return $context.$data.select; }, 'css': function () { return { 'dxd-state-selected dxd-back-secondary': $context.$data.selected }; } }; });
...
class _columnDragHandler_ColumnDragHandler extends external_DevExpress_Analytics_Internal_["DragDropHandler"] {
    ...
    startDrag(control) {
        ...
        const connectorModel = new routedConnectorModel_RoutedConnectorViewModel({}, this.querySurface().getControlModel());
...
class _dbObjectDragDropHandler_DbObjectDragDropHandler extends external_DevExpress_Analytics_Internal_["DragDropHandler"] {
    constructor(surface, selection, _undoEngine, snapHelper, dragHelperContent) {
        ...
        this.getDropCallback = (undoEngine, suggestLocation) => ((item, query) => {
            const newControl = query.createChild(Object(external_DevExpress_Analytics_Internal_["extend"])({ '@ControlType': 'Table', '@Name': item.data.name }, controlsFactory.controlsMap['Table'].defaultVal));
    ...
    doStopDrag(uiElement, _) {
        ...
        if (this.selection.dropTarget) {
            ...
            const item = external_ko_["dataFor"](uiElement);
    ...
    addControl(control, dropTargetSurface, size) {
        ...
        const controlSurface = Object(external_DevExpress_Analytics_Internal_["findSurface"])(control);
        if (!controlSurface)
            return;
...
class _accordionTabInfo_AccordionTabInfo extends external_DevExpress_Analytics_Utils_["TabInfo"] {
    ...
    static _createWrappedObject(query, commonModel, undoEngine, showParameters) {
        const modelProperties = new external_DevExpress_Analytics_Widgets_["ObjectProperties"](query, null, 1);
        const modelValues = external_ko_["computed"](() => query() && query().parameters);
    ...
    static _createQBPropertyGrid(query, commonModel, undoEngine, showParameters) {
        ...
        const grid = new external_DevExpress_Analytics_Internal_["ControlProperties"](external_ko_["observable"](object), {
...
external_ko_["bindingHandlers"]['dxdTableView'] = {
    init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
        const templateHtml = Object(external_DevExpress_Analytics_Widgets_["getTemplate"])('dxd-tableview');
        let $element = external_jQuery_["fn"].constructor(element).append(templateHtml), $titles, $content;
        const rtl = !!bindingContext.$root.rtl, value = external_ko_["unwrap"](valueAccessor());
        ...
        $element.find('.dxd-tableview-titles .dxd-tableview-resizable').each((index, resizable) => {
            const $title = external_jQuery_["fn"].constructor(resizable).find('.dxd-tableview-cell-text');
            const $column = $element.find('.dxd-tableview-data .dxd-tableview-resizable' + index);
            if (index < value.schema.length - 1) {
                const resizableElement = new external_DevExpress_Analytics_Internal_["Resizable"](resizable, {
                    handles: 'e',
                    start: (e, ui) => { },
                    stop: () => { },
                    resize: (e, element) => {
...
var external_DevExpress_config_ = __webpack_require__(12);
...
function customizeDesignerActions(designerModel, nextCustomizer) {
    ...
    return ((actions) => {
        const del = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(actions, action => action.text === 'Delete');
        del.imageClassName = 'dx-icon-dxrd-image-recycle-bin';
        del.imageTemplateName = 'dxrd-svg-operations-recycle_bin_xl';
        const undo = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(actions, action => action.text === 'Undo');
        undo.disabled = external_ko_["pureComputed"](() => designerModel.isLoading() || !designerModel.undoEngine().undoEnabled());
        const redo = Object(external_DevExpress_Analytics_Internal_["findFirstItemMatchesCondition"])(actions, action => action.text === 'Redo');
        actions.splice(0, actions.length, del, undo, redo);
        actions.push({
            id: ActionId.Save,
            text: 'Save',
...
function createIsLoadingFlag(model, dbSchemaProvider) {
    const isDbSchemaLoaded = external_ko_["observable"](false);
...
function _createDesignerModel(query, surface, options) {
    ...
    const selection = new external_DevExpress_Analytics_Internal_["SurfaceSelection"](['alias', 'name', 'sortOrder']);
    const designer = Object(external_DevExpress_Analytics_Internal_["createDesigner"])(query, surface, controlsFactory, undefined, undefined, undefined, !!options.rtl, selection);
    designer.addDisposables(querySubscription);
    designer.findControl = (s, e) => {
        const $childs = external_jQuery_["fn"].constructor('.dxqb-main').children('.dxrd-control');
        $childs.each((_, child) => {
            const $child = external_jQuery_["fn"].constructor(child);
...
function _updateSurfaceContentSizeSubscription(element, designerModel, surface, callback) {
    const updateSurfaceContentSize_ = updateQueryBuilderSurfaceContentSize(() => external_jQuery_["fn"].constructor(element).find('.dxqb-designer'), designerModel.surfaceSize, surface);
...
function _createQueryBuilder(element, options, callbacks, applyBindings = true) {
    ...
    const wrapper = options.requestWrapper || new requestwrapper_RequestWrapper();
    ...
    if (options.queryModelJson) {
        const dataSource = new sqlDataSource_SqlDataSource(JSON.parse(options.dataSourceJson));
    ...
    const query = external_ko_["observable"](), surface = external_ko_["observable"](), treeListOptions = external_ko_["observable"]();
    ...
    const previewPopupContainer = external_DevExpress_Analytics_Internal_["getParentContainer"];
    ...
    const tablesTop = external_ko_["observable"](Object(external_DevExpress_Analytics_Internal_["calculateWithZoomFactor"])(355)), itemPropertiesTabInfoModel = {
    ...
    const tabPanelItem = new _accordionTabInfo_AccordionTabInfo(query, itemPropertiesTabInfoModel, designerModel.undoEngine, designerModel.selection.focused, parametersMode === ParametersMode.ReadWrite);
...
function createQueryBuilder(element, options, callbacks, applyBindings = true) {
    ...
    return Object(external_DevExpress_Analytics_Internal_["resolveFromPromises"])(promises, () => {
        return Object(external_DevExpress_Analytics_Internal_["troubleshootingPageWrapper"])(() => _createQueryBuilder(element, options, callbacks, applyBindings), options.developmentMode, element);
    });
}
function createQueryBuilderSurface(element, options, queryCreator) {
    const query = external_ko_["observable"](), surface = external_ko_["observable"]();
    _controlsFactory_registerControls();
    const designerModel = _createDesignerModel(query, surface, options);
    const selectedTabPanel = new _accordionTabInfo_SelectedTabInfo(new external_DevExpress_Analytics_Widgets_["ObjectProperties"](designerModel.editableObject, null, 1));
    const tabPanel = designerModel.tabPanel;
    const collased = external_ko_["observable"](tabPanel.collapsed);
    let isUpdate = false;
    const lock = (callback) => {
        if (!isUpdate) {
            isUpdate = true;
    ...
    const switcher = new _rightPanelSwitcher_RightPanelSwitcher(collased, designerModel.editableObject, options.showPropertyGridCondition, _accordionTabInfo_AccordionTabInfo._getSelectedItemPropertyName);
...
var external_DevExpress_localization_ = __webpack_require__(13);

// EXTERNAL MODULE: external "DevExpress.Analytics.Localization"
var external_DevExpress_Analytics_Localization_ = __webpack_require__(14);
...
class jsQueryBuilderBinding_JSQueryBuilderBinding extends external_DevExpress_Analytics_Internal_["JSDesignerBindingCommon"] {
    ...
    _applyBindings(model, _$element) {
        Object(external_DevExpress_Analytics_Internal_["troubleshootingPageWrapper"])(() => {
    ...
    _initializeCallbacks() {
        if (this._options.callbacks) {
            const queryBuilderEvents = eventGenerator_EventGenerator.generateQueryBuilderEvents((eventName, args) => {
    ...
    applyBindings(element) {
        const _$element = external_jQuery_["fn"].constructor(element);
...
external_ko_["bindingHandlers"][queryBuilderBindingName] = {
    init: function (element, valueAccessor) {
        const values = external_ko_["unwrap"](valueAccessor());
...
external_ko_["bindingHandlers"]['dxQueryBuilderSurface'] = {
    init: function (element, valueAccessor) {
        ...
        const templateHtml = Object(external_DevExpress_Analytics_Widgets_["getTemplate"])('dxrd-querybuilder'), $element = external_jQuery_["fn"].constructor(element).append(templateHtml);
